...
This document contains code for the modelling examples (case studies) included in the paper 'An introduction to generative network models and their use in animal sociality research' by Brask, Silk & Weiss.

# Set-up

To run these examples, you'll need to first install and then load several R libraries.

Some of the examples use the okaapibeta package (the beta version of the okaapi package), which generates networks based on the trait preference model (see these references in the main text: Brask et al. 2025 and Brask et al. in press). This package is available at Github and can be installed by the following procedure: first install the remotes package (only necessary if you don't have this or the devtools package installed already), then load that package (to get access to the install_github function), then run the following code in R: install_github("bohrbrask/okaapibeta"). The other packages can be installed in the standard way. 
...
Here, we give a demonstration of how to use generative network models to investigate the emergence of animal social networks. 

We want to investigate how generative processes - here social preferences - affect network structure. Specifically, We investigate how social preferences for individuals similar to oneself (homophily) affect social network structure in a social system where there are five matrilines and individuals prefer to socialize with those of their own matriline. 
...
of preferences (which are zero for the random networks).
...
allncats <- 5                                     # trait category number. We use five categories (five matrilines).
...
Here, we demonstrate how to use generative models to investigate dynamics on and of animal social networks.

We are interested in the link between social network structure and disease spread. First, let's set up a function to generate networks using the Erdös-Rényi algorithm, which produces random networks with a specified link probability:
...
Here, we present an example of how to investigate the robustness of animal social networks with generative network modelling.
...
We investigate robustness to different types of loss of individuals (node removal), where individuals (nodes) are removed either in random order or in order depending on their connectedness.

Note: the code produces results for all network types and for one node removal type. To produce results for multiple node removal types (such as in the main text), we would need to run the code one time for each node removal type.
...
netsurvival <- function(netbreaktimes, n, nrepls) { 
  timestepsn <- n-1 # because time 0 is also a time step
...
```{r robustnessplot, include = TRUE, echo=TRUE, cache=TRUE}
...
# make box plot
plotdata <- breaktimes
colnames(plotdata)<- c('RD','SF','MD') # change to labels that refer to network structure
...
# make survival plot 
removednodespc <- 0:(n-2) # percentage of nodes removed per time step (when n=100)
...
In this final simulation, we demonstrate how to use generative network models for studies of animal social network methodology.

We'll show how generative network models can be used to assess statistical power in animal social network analysis. Specifically, we'll be investigating how likely a statistical analysis is to detect assortment in the network, given different true levels of assortment and sampling effort.
